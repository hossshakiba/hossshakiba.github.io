...
const Cart = (props) => {
  ...
  return (
    <div
      className={`group relative shadow-4xl md:hover:shadow-3xl cursor-pointer grid self-center sm:grid-cols-3 mb-4 md:mb-8 w-full md:w-[90%] lg:w-[80%] xl:w-[85%] 2xl:w-[70%] p-4 md:p-5 rounded-lg border lg:transition lg:ease-in-out lg:delay-150 lg:hover:-translate-y-1 lg:hover:scale-[1.01] ${
        featured
          ? 'pub-featured'
          : 'theme-surface'
      }`}
    >
      ...
      {hasAbstract && (
        <button
          ...
          aria-label={abstractOpen ? 'Hide abstract' : 'Show abstract'}
          title={abstractOpen ? 'Hide abstract' : 'Show abstract'}
          onClick={(e) => {
            e.stopPropagation();
            setAbstractOpen((open) => !open);
          }}
          className={`pub-abstract-corner ${abstractOpen ? 'is-open' : ''}`}
        >
          <HiChevronDown className="pub-abstract-chevron" aria-hidden />
        </button>
      )}
      ...
      <div className="sm:col-span-2 flex flex-col justify-between ml-1.5 sm:ml-2 md:ml-8">
        ...
        <div className="[&>*]:mt-2 md:[&>*]:mt-3 flex flex-wrap [&>*]:mr-2 mb-0 sm:mb-2">
          {(props.data.projectPage || props.data.website) &&
            <Link href={props.data.projectPage || props.data.website || props.data.url}>
              <button className={`${actionButtonClassName} hover:border-[#EB8317] hover:text-[#EB8317]`}>
                <IoEarthSharp />
                <span className="text-xs ml-1">Project Page</span>
              </button>
            </Link>
          }
          {props.data.pdf &&
